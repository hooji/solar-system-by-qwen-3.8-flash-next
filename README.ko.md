...
### ▶ [앱 실행하기](https://claude.ai/code/artifact/5135e079-986e-4779-bc97-82a03b6b064d) — 설치 없이 브라우저에서 바로 실행됩니다

(로컬 실행을 원하면 [최신 릴리스](../../releases/latest)의 단일 HTML 파일을 받아 여세요.)
...
- 별배경은 실제 밤하늘: 예일 밝은 별 목록(BSC5, 9,096개 육안 별 전부)의 J2000 좌표·실제 등급·B−V 색을 사용, 황도 기준으로 정확히 정렬(황도경사각 23.44° 회전, 거울상 없음)
...
빌드 없이 바로 실행하려면 [최신 릴리스](../../releases/latest)에서 받을 수
있습니다: 단일 HTML 파일(`solar-system-<버전>.html`)은 텍스처까지 모두 내장돼
있어 브라우저로 바로 열면 되고, zip은 압축을 풀어 아무 정적 서버로 서빙하면
됩니다. 릴리스는 `v*` 태그 푸시 또는 Actions의 Release 워크플로 수동 실행
(최신 버전 자동 증가, 태그가 없으면 v1.0.0)으로 만들어집니다.
