...
- 전환 시 헤더·인포를 넘어 패널 전체가 따라간다(t_292b0645): 제어 패널의
  ...
  aria-label까지 사전 조회로 즉시 다시 렌더링된다. 개발자 로그는 번역 대상이
  아니다. 실데이터 값도 언어별 우선순위를 따른다(t_8701c121): 한국어 모드는
  기존 한/영 병기·단위 문구 그대로, EN 모드는 `Jupiter · 목성`·`11.86 years
  (4,333 days)`처럼 영어가 앞에서 리드하고 화면 이름표도 줄 순서가 바뀐다.
  두 이름·두 단위 표기(km/AU)는 어떤 언어에서도 항상 함께 보이고, 숫자·환산
  값·경사/스케일 계산은 언어와 무관하게 동일하다(표시 계층만 변경). 라벨
  표시/숨김과 병기 여부는 언어 토글과 독립적으로 계속 동작한다.
...
- 패널 언어(t_00139ab5): `ui/i18n.ts`가 단일 언어 기반. 사전 키 집합의 참은
  ...
  CDP, 52 항목 — 헤더뿐 아니라 각 패널의 양언어 렌더링·aria 전환, 실데이터
  라벨 우선순위·라벨 줄 순서 전환, 라벨 가시성 독립 포함).
...
- 라벨·인포 패널(t_d9203468): 실제값 표시 규칙은 `ui/format.ts`가 유일
  (순수·Node 테스트 `scripts/info-format.test.mjs` 19항목). 누락 데이터는
  ...
  단일 규칙(t_8701c121): 한국어 모드 "목성 · Jupiter", EN 모드
  "Jupiter · 목성" — 두 이름은 항상 병기, 주기·역행 단위 WORD만 언어별이며
  (`unit.*`/`rotation.retrograde` 사전 키) 숫자는 양언어 동일. `InfoPanel`은 실데이터 섹션과 렌더 값
